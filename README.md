...
- macOS 13 ขึ้นไป
- อินเทอร์เน็ตครั้งแรก เพื่อติดตั้ง Python/ไลบรารีที่จำเป็นและดาวน์โหลดโมเดล (~1.5 GB)
  — หลังจากนั้นใช้งานออฟไลน์ได้

ตัวติดตั้งจะตรวจสอบและติดตั้ง **Python 3.13** จาก python.org ให้อัตโนมัติหากเครื่องยังไม่มี
(ดาวน์โหลดเพิ่ม ~68 MB และ macOS จะถามรหัสผ่านผู้ดูแลระบบ) จากนั้นจะสร้าง virtualenv
และติดตั้งไลบรารีทั้งหมดจาก `requirements.txt` ให้เอง
...
4. เมื่อเสร็จ ตัวติดตั้งจะเปิด `FastWhisper Toggle.app` เพื่อให้ macOS ลงทะเบียน `Python.app`
   แล้วเปิดหน้า Accessibility ให้เอง → เปิดสวิตช์ **Python.app** เป็น **ON**
   (`FastWhisper Toggle.app` ไม่จำเป็นต้องอยู่ในรายการ Accessibility)
5. หาก `Python.app` ไม่ขึ้นอัตโนมัติ ให้กด **+** → **⌘⇧G** → **⌘V** เพื่อวางพาธที่ตัวติดตั้งคัดลอกไว้
   จากนั้นรอไอคอน 🎙 ใน menu bar
...
- แอปจำกัดและคืน MLX/Metal cache อัตโนมัติหลังถอดเสียง เพื่อลดแรงกดดันต่อ unified memory
- หากต้องคืนหน่วยความจำของโมเดลทันที เลือก `Release Memory` จาก menu bar
  (ครั้งถัดไปจะช้าขึ้นเล็กน้อยเพราะโหลดโมเดลจากดิสก์ใหม่)
- แก้ปัญหาเบื้องต้นจาก menu bar ได้: `Restart Listener`, `Test Mic`, `Release Memory`, `Open Log`
...
5. เลือก **BlackHole 2ch** ทางซ้าย แล้วดูแท็บ Input ให้ปุ่ม **Mute ปิดอยู่**
   (ตัวติดตั้งและแอปเวอร์ชันใหม่จะปลด mute ให้อัตโนมัติ)

### ขั้นที่ 3 — Sound Output จะสลับให้อัตโนมัติ

แอปจะเลือก **Multi-Output Device** ให้อัตโนมัติเมื่อเริ่มอัด และกลับไปใช้
output เดิมเมื่อปล่อยปุ่ม โดยใช้ `SwitchAudioSource` ที่ installer ติดตั้งให้
...
| เห็น ⚠️ ที่ menu bar หลังปล่อยปุ่ม | เปิดเมนูแอปดู Error; ถ้า Multi-Output หาไม่เจอ ให้ทำขั้นที่ 2 ให้ครบ |
| ได้ข้อความมั่ว ๆ ซ้ำ ๆ เช่น "nenenene" | อัดได้แต่ความเงียบ — สาเหตุเดียวกับข้อบน |
| `peak 0.0000` หลังรีบูต | BlackHole อาจจำสถานะ Mute ไว้ — แอปเวอร์ชันใหม่จะปลดให้อัตโนมัติ หรือเปิด Audio MIDI Setup → BlackHole 2ch → Input → ปิด Mute |
...
| ใช้นานแล้วหน่วยความจำสูง | เลือก `Release Memory` จาก menu bar เพื่อ unload โมเดลและคืน MLX cache ให้ macOS |
